...
function transformModel(options, model) {
    ...
    itr(model.attributes, (attr, name) => {
        ...
        /*
        //BUG: This will always set fields default value even if we
        //don't want to include it in the response or in the PUT body.
        ...
        if (attr.type === 'text') property.type = 'text';
        ...
        //Old waterline formatting of validations: https://github.com/balderdashy/waterline-docs/blob/7a87ba4ab08048ef06c3cc1f8b11baa70ed13a6b/models/validations.md
        formats.map(format => {
            if (attr.hasOwnProperty(format)) property.format = format;
        });

        if (attr.format) property.format = attr.format;
